...
function displayCharacterInfo(characterData) {
    ...
    aragornData.textContent = `${Aragorn.name} is a ${Aragorn.race} born in ${Aragorn.birth} and died in ${Aragorn.death}. He played a pivotal role in helping Frodo get the ring to mount doom, and in defending his allies right up until the end. Extra info found here: ${Aragorn.wikiUrl}`
    characters.appendChild(aragornData)
    arwenData.textContent = `${Arwen.name} is an ${Arwen.race} born in ${Arwen.birth} and died in ${Arwen.death}. She helped Frodo escape from orcs and was Aragorn's bride. Extra info found here: ${Arwen.wikiUrl}`
    characters.appendChild(arwenData)
    bilboData.textContent = `${Bilbo.name} is a ${Bilbo.race} born on ${Bilbo.birth} and his death is ${Bilbo.death}. He is Frodo's uncle and was known to carry the ring of power for a time, but courageously gave it up to be destroyed. A well known previous member of the Shire. Extra info found here: ${Bilbo.wikiUrl}`
    characters.appendChild(bilboData)
    eowynData.textContent = `${Eowyn.name} is a Human born in ${Eowyn.birth} and died in ${Eowyn.death}. She was known throughout the land to be an exceptionally brave and stalwart young woman, who defeated the Witch King of Angmar, one who boldly declared that no man could defeat. Extra info found here: ${Eowyn.wikiUrl}`
    characters.appendChild(eowynData)
    faramirData.textContent = `${Faramir.name} is a ${Faramir.race} born in ${Faramir.birth} and died in ${Faramir.death}. He was a noble man who fought not only for Gondor and her people, but for the love of his father. Extra info found here: ${Faramir.wikiUrl}`
    characters.appendChild(faramirData)
    frodoData.textContent = `${Frodo.name} is a ${Frodo.race} born on ${Frodo.birth} and his death is ${Frodo.death}. He was the one who bore the burden of taking the one ring to the fiery pits of mount doom, in exchange for great pain and sacrifice along the way. Extra info found here: ${Frodo.wikiUrl}`
    characters.appendChild(frodoData)
    galadrielData.textContent = `${Galadriel.name} is an ${Galadriel.race} born in ${Galadriel.birth} and is ${Galadriel.death}. She is the elf queen and helped the fellowship of the ring on their journey, and who had the power to warn Frodo of his difficult future. Extra info found here: ${Galadriel.wikiUrl}`
    characters.appendChild(galadrielData)
    gandalfData.textContent = `${Gandalf.name} is a ${Gandalf.race} born ${Gandalf.birth} and died in ${Gandalf.death}. He was an incredible asset to both Frodo and Sam on their way to mordor, as well as in defeating Sauron's armies. A truly legendary man who defied the impossible, and often took matters into his own hands to change the course of history. Extra info found here: ${Gandalf.wikiUrl}`
    ...
    gollumData.textContent = `${Gollum.name} is a ${Gollum.race} born in ${Gollum.birth} and died on ${Gollum.death}. He was greedy and evil, yet valuable and knowledgable all at the same time. Many hated him, some cared for him, but all who met him surely pitied him. He was a tragic example of one who sought for that which inevitably became his downfall. Extra info found here: ${Gollum.wikiUrl}`
    characters.appendChild(gollumData)
    legolasData.textContent = `${Legolas.name} is an ${Legolas.race} born at an unknown date, but is known to be ${Legolas.death}. He was always at the side of Aragorn and Gimli, and became a great friend to them, always plunging himself into the forefront of battle. It is truly amazing what he can do with merely a bow and a few arrows. A fearless elf with great great abilities and a strong resolve. Extra info found here: ${Legolas.wikiUrl}`
    characters.appendChild(legolasData)
    samData.textContent = `${Sam.name} is a ${Sam.race} born on ${Sam.birth} and is known to be ${Sam.death}. He stayed by Frodo's side at every footstep to help him carry the ring to be destroyed. He is someone you can rely on, someone who you know will never give up on you. And that he did for Frodo, even when all seemed lost. He is a beautiful example of one who will endure any hardship to save his friends. Extra info found here: ${Sam.wikiUrl}`
